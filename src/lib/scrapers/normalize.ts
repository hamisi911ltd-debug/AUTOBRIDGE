...
export function guessBodyType(modelText: string): string {
  ...
  if (
    /\b(VAN|HIACE|CARAVAN|NV350|ALPHARD|VELLFIRE|NOAH|VOXY|SERENA|STEPWAGON|FREED|SIENTA|ELGRAND)\b/.test(s)
  )
    return "Van";
  ...
  if (
    /\b(RAV4|HARRIER|LAND CRUISER|PRADO|CX-5|CX-3|FORESTER|X-TRAIL|CR-V|VEZEL|HR-V|SPORTAGE|OUTLANDER|KLUGER|HIGHLANDER|PAJERO)\b/.test(
      s
    )
  )
    return "SUV";
