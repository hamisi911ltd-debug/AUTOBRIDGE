...
// Cropped to a shared height (2172x320, vs. the source PNGs' own 2172x395)
// so the auto-slide never jumps in size between slides. Each PNG was
// designed with a plain-background margin top and bottom (most visible on
// the "DISCOUNT" slide), so object-cover's default centered crop trims 38px
// of that dead space off each edge. Checked against all three source images
// with a one-off crop preview before picking this number: it's the largest
// crop that doesn't clip real content on the tightest slide ("IMPORT
// FROM..." / "DEDICATED SUPPORT" on promo-banner-3 starts getting cut at
// ~50px per side).
const BANNERS = [
...
export function PromoBanner({ onGoSearch }: { onGoSearch: () => void }) {
  ...
  return (
    <div>
      ...
      >
        <button onClick={onGoSearch} className="block w-full relative aspect-[2172/320]" aria-label="Search vehicles">
          {BANNERS.map((b, i) => (
